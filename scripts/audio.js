...
panValue = 0.5;
volumeValue = 1;
dataset = [highpassValue, volumeValue];
...
function init() {
    ...
    initSpectrum();
    $('#play2')[0].onClick = function(){
        console.log('clickkkk');
        toggleSound();
    }
    $('#volume2')[0].onTurn = function(){
        console.log('turned');
        volumeValue = this.angle;
                     myNodes.volume.gain.value = volumeValue;

        drawSliders();
    }
        $('#pass2')[0].pressureChanged = function(){
        highpassValue = this.pressure*2000;
         myNodes.filter.frequency.value = highpassValue;

        drawSliders();
    }
    //  $('#gain2')[0].onBend = function(){
    //     highpassValue = this.angle/90*255;
...
function drawSliders(){
    d3.select("body").selectAll("#slider")
        .data([highpassValue/2000, volumeValue])
        .enter();
    d3.select("body").selectAll("#slider")
...
function routeSound(source) {
    ...
    myNodes.filter.frequency.value = highpassValue;
    myNodes.panner.setPosition(panValue, 0, 0);
    myNodes.volume.gain.value = volumeValue;
